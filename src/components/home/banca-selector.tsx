				...
				{visibleBancas.map((banca) => {
					if (banca.comingSoon) {
						return (
							<div
								className={cn(
									"group relative flex flex-col items-center justify-center rounded-xl border border-zinc-200 bg-white p-8 transition-all duration-200 md:p-12",
									"cursor-not-allowed opacity-60",
								)}
								key={banca.id}
							>
								<div className="absolute top-4 right-4">
									<span className="rounded-full bg-zinc-900 px-3 py-1 font-medium text-white text-xs">
										Coming soon
									</span>
								</div>
								...
								<p className="mt-6 font-medium text-base text-zinc-900 md:text-lg">
									{banca.name}
								</p>
							</div>
						);
					...
					return (
						<Link href={banca.route} key={banca.id}>
							<div className="group relative flex flex-col items-center justify-center rounded-xl border border-zinc-200 bg-white p-8 transition-all duration-200 hover:shadow-lg hover:shadow-zinc-200/50 md:p-12">
								<div className="relative h-24 w-full md:h-32">
									<Image
										alt={`Logo ${banca.name}`}
										className="object-contain"
							...
							(pageIdx) => (
								<button
									...
									className={cn(
										...
										pageIdx === currentPage ? "w-6 bg-zinc-900" : "bg-zinc-300",
									)}
									key={pageIdx}
